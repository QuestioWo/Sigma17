...
function parseAsmLineSplitter( st ) {
    lineDict = {
        'FIELDLABEL' : '',
        'FIELDOPERATION' : '',
        'FIELDOPERAND' : '',
    ...
    // split so array of chars
    stlist = st.trim().split('')
    ...
    for ( var i = 0; i < stlist.length; i++ ) {
        if ( stlist[i] === ';' ) {
            if ( i+1 < stlist.length && stlist[i+1] == ';' ) {
                if ( i+2 < stlist.length ) {
                    lineDict['FIELDCOMMENT'] = stlist.join('').substring( i+2 )
                }
                break;
            } else {
                labelling = true;
        ...
        if ( labelling && stlist[i] !== ' ' ) {
            lineDict['FIELDLABEL'] += stlist[i];
        } else {
            if ( whitespacing ) {
                whitespacing = false;
                whitespaceCounter ++
            ...
            if ( whitespaceCounter === 0 ) {
                lineDict['FIELDOPERATION'] += stlist[i]
                continue;
            } else if ( whitespaceCounter === 1 ) {
                lineDict['FIELDOPERAND'] += stlist[i]
            }
        }
    }
...
function parseAsmLine (m,i) {   
    let s = m.asmStmt[i];
    let p = parseAsmLineSplitter(s.srcLine);
    s.fieldDict = p
    s.fieldOriginal = s.srcLine
    s.fieldLabel = p['FIELDLABEL'];
    s.fieldOperation = p['FIELDOPERATION'];
    s.fieldOperands = p['FIELDOPERAND'];
    s.fieldComment = p['FIELDCOMMENT'];
...
function asmPass2 (m) {
    ...
    // displayed in processor for the fully highlighted text when being parsed
    ...
    // displayed in assembler and in non-highlighted over text in processor
    s.listingLineHighlightedFields = (s.lineNumber+1).toString().padStart(4,' ')
        + ' ' + wordToHex4(s.address)
        + ' ' + (s.codeSize>0 ? wordToHex4(s.codeWord1) : '    ')
        + ' ' + (s.codeSize>1 ? wordToHex4(s.codeWord2) : '    ')
    for ( var key in s.fieldDict ) {
        fieldValue = s.fieldDict[key]
        if ( key === 'FIELDLABEL' && fieldValue.trim() !== '' ) {
            fieldValue = ';' + fieldValue
        } else if ( key === 'FIELDCOMMENT' && fieldValue.trim() !== '' ) {
            fieldValue = ';;' + fieldValue
        }

        if ( fieldValue.trim() !== '' ) {
            s.listingLineHighlightedFields = s.listingLineHighlightedFields 
            + ' ' + highlightField( fieldValue, key )
        }
    }
